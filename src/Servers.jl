"""
The `HTTPA.Servers` module provides HTTPA server functionality.

The main entry point is `HTTPA.listen(f, host, port; kw...)` which takes
a `f(::HTTPA.Stream)::Nothing` function argument, a `host`, a `port` and
optional keyword arguments.  For full details, see `?HTTPA.listen`.
"""
module Servers
...
"""
    HTTPA.listen([host=Sockets.localhost[, port=8081]]; kw...) do http
        ...
    end

Listen for HTTPA connections and execute the `do` function for each request.

The `do` function should be of the form `f(::HTTPA.Stream)::Nothing`.
...
e.g.
```
    HTTPA.listen("127.0.0.1", 8081) do http
        HTTPA.setheader(http, "Content-Type" => "text/html")
...
`HTTPA.listen`. This allows control of server shutdown.

e.g.
```
    server = Sockets.listen(Sockets.InetAddr(parse(IPAddr, host), port))
    @async HTTPA.listen(f, host, port; server=server)

    # Closeing server will stop HTTPA.listen.
    close(server)
```

To run the following HTTPA chat example, open two Julia REPL windows and paste
...
chat_server() = HTTPA.listen("127.0.0.1", 8087) do io
    write(io, "HTTPA.jl Chat Server. Welcome!")
    chat(io)
end

chat_client() = HTTPA.open("POST", "http://127.0.0.1:8087") do io
    chat(io)
end
```
"""
...
Create a `Transaction` object for each HTTPA Request received.
...
"""
Create a `HTTPA.Stream` and parse the Request headers from a `HTTPA.Transaction`
