...
`HTTPA.Multipart` type is provided to support setting the `Content-Type`, `filename`, and `Content-Transfer-Encoding` if desired. See `?HTTPA.Multipart` for more details.
"""
...
function writemultipartheader(io::IOBuffer, i::IOStream)
    ...
    write(io, "Content-Type: $(HTTPA.sniff(i))\r\n\r\n")
    return
    end
...
"""
Multipart(filename::String, data::IO, content_type=HTTPA.sniff(data), content_transfer_encoding="")

A type to represent a single multipart upload chunk for a file. This type would be used as the value in a
key-value pair of a Dict passed to an http request, like `HTTPA.post(url; body=Dict("key"=>HTTPA.Multipart("MyFile.txt", open("MyFile.txt"))))`.
...
of the `HTTPA.sniff(data)` mimetype detection algorithm, whereas `Content-Transfer-Encoding` will be left out if not specified.
"""
...
Base.show(io::IO, m::Multipart{T}) where {T} = print(io, "HTTPA.Multipart(filename=\"$(m.filename)\", data=::$T, contenttype=\"$(m.contenttype)\", contenttransferencoding=\"$(m.contenttransferencoding)\")")
...
function writemultipartheader(io::IOBuffer, i::Multipart)
    ...
    contenttype = i.contenttype == "" ? HTTPA.sniff(i.data) : i.contenttype
