...
"""
    request(RetryLayer, ::URI, ::Request, body) -> HTTPA.Response
...
e.g. `HTTPA.IOError`, `Sockets.DNSError`, `Base.EOFError` and `HTTPA.StatusError`
(if status is ``5xx`).
"""
...
isrecoverable(e::HTTPA.StatusError) = e.status == 403 || # Forbidden
...
function no_retry_reason(ex, req)
    ...
    print(buf, ", ",
        ex isa HTTPA.StatusError ? "HTTPA $(ex.status): " :
