...
"""
`HTTPA.sniff(content::Union{Vector{UInt8}, String, IO})` => `String` (mimetype)

`HTTPA.sniff` will look at the first 512 bytes of `content` to try and determine a valid mimetype.
If a mimetype can't be determined appropriately, `"application/octet-stream"` is returned.

Supports JSON detection through the `HTTPA.isjson(content)` function.
"""
function sniff end
